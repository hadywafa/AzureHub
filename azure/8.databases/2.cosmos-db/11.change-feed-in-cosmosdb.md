...
Whether you use Functions or Processor library, four core pieces exist:

1. **Monitored Container** – the source container where changes occur.
2. **Lease Container** – tracks progress per partition (leases = checkpoints).
3. **Compute Instance(s)** – your code consuming changes.
4. **Delegate/Handler** – custom logic to process each batch of changes.
...
```mermaid
flowchart TD
    A[Cosmos DB - Monitored Container] -->|Change Feed| B[Lease Container]
    B --> C[Change Feed Processor / Azure Function]
    C --> D[Delegate - Your Custom Logic]
    D --> E[(Downstream Systems: Cache, Queue, Analytics, API)]
```
