...
<div align="center">
  <img src="image/1.azure-function-overview/1758301502358.png" alt="Azure Functions Overview" style="border-radius: 10px; width: 100%; border: 2px solid white">
</div>
...
```mermaid
flowchart LR
    User["Event Source 📨"] --> Trigger["Azure Function Trigger ⚡"]
    Trigger --> Func["Azure Function Code 📝"]
    Func --> Binding["Bindings 🔗 (Input/Output)"]
    Binding --> Target["Target Services: DB, Queue, API, Blob"]
```
...
| --------------------------- | ---------------------------------------- | ----------------------------- |
| **Consumption Plan**        | Default, pay-per-use, auto-scale         | Cold start possible ❄️        |
| **Premium Plan**            | For predictable workloads, no cold start | Auto-scale + VNET integration |
| **Dedicated (App Service)** | Runs on App Service Plan (fixed cost)    | Manual scaling                |
